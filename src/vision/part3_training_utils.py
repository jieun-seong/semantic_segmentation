...
def get_model_and_optimizer(args) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    if args.arch == "PSPNet":
        model = PSPNet(pretrained=args.pretrained, num_classes=n_classes)
        ...
        model = SimpleSegmentationNet(pretrained=args.pretrained, num_classes=n_classes)
        optimizer = torch.optim.SGD([
            {'params': model.layer0.parameters(), 'lr': args.base_lr, 'momentum': mmt, 'weight_decay': w_d}, 
            {'params': model.resnet.layer1.parameters(), 'lr': args.base_lr, 'momentum': mmt, 'weight_decay': w_d}, 
            {'params': model.resnet.layer2.parameters(), 'lr': args.base_lr, 'momentum': mmt, 'weight_decay': w_d}, 
            {'params': model.resnet.layer3.parameters(), 'lr': args.base_lr, 'momentum': mmt, 'weight_decay': w_d}, 
            {'params': model.resnet.layer4.parameters(), 'lr': args.base_lr, 'momentum': mmt, 'weight_decay': w_d}, 
            {'params': model.cls.parameters(), 'lr': args.base_lr*10.0, 'momentum': mmt, 'weight_decay': w_d}
        ])
...
def get_train_transform(args) -> transform.Compose:
    ...
    # namespace(ignore_label=255, rotate_max=10, rotate_min=-10, scale_max=2.0, 
    # scale_min=0.5, short_size=240, train_h=201, train_w=201)
