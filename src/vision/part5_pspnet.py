...
class PSPNet(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            zoom_factor: indicates how "zoomed in" the final feature map is 
                compared to the input image. 
                zoom_factor of 8 means that the final feature map size is 
                1/8 of the input image.
                This means that if your zoom_factor is not 1, 
                then you would have to interpolate your feature map.
        ...
        if use_ppm:
            fea_dim = 2048
            in_dim = fea_dim
            reduction_dim = fea_dim // len(self.bins)
            self.ppm = PPM(in_dim, reduction_dim, self.bins)
            fea_dim = in_dim + len(self.bins)*reduction_dim
        else:
            fea_dim = 2048
